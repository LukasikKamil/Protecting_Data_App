...
{
    // salt size must by at least 8 bytes, we will use 16 bytes
    private static readonly byte[] salt = Encoding.Unicode.GetBytes("7BANANAS");

    // Default iteriations for Rfc2898DeriveBytes is 1000.
    // Iterations should by high enough to take at least 100ms to
    // generate a Key and IV on the target machine. 150,000 iterations
    // takes 139ms on 11th Gen Intel Core i7-1165G7 @ 2.80GHz.
    ...
    public static string Encrypt(string plainText, string password)
    {
        ...
        using (Aes aes = Aes.Create()) // abstract class factory method 
        {
            ...
            {
                WriteLine("PBKDF2 algorithm: {0}, Iterations count: {1:N0}",
    ...
    public static void LogIn(string username, string password)
    {
        ...
        {
            GenericIdentity gi = new(name: username, type: "AiOAuth");
